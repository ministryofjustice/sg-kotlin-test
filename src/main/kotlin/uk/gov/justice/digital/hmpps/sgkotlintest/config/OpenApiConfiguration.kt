package uk.gov.justice.digital.hmpps.sgkotlintest.config
...
class OpenApiConfiguration(buildProperties: BuildProperties) {
  ...
  fun customOpenAPI(): OpenAPI = OpenAPI()
    .servers(
      listOf(
        Server().url("https://sg-kotlin-test-dev.hmpps.service.justice.gov.uk").description("Development"),
        Server().url("https://sg-kotlin-test-preprod.hmpps.service.justice.gov.uk").description("Pre-Production"),
        Server().url("https://sg-kotlin-test.hmpps.service.justice.gov.uk").description("Production"),
    ...
    .info(
      Info().title("Sg Kotlin Test").version(version)
    ...
    .components(
      Components().addSecuritySchemes(
        "sg-kotlin-test-ui-role",
        SecurityScheme().addBearerJwtRequirement("ROLE_TEMPLATE_KOTLIN__UI"),
      ),
    )
    .addSecurityItem(SecurityRequirement().addList("sg-kotlin-test-ui-role", listOf("read")))
}
